...
SQAction::SQAction(const cell<QString> &text, const sodium::cell<bool> &enabled, QObject *parent)
    : QAction(parent)
{
    m_unsubscribe += text.listen(
        ensureSameThread<QString>(this, [this](const QString &s) { setText(s); }));
    m_unsubscribe += enabled.listen(ensureSameThread<bool>(this, [this](bool b) { setEnabled(b); }));
